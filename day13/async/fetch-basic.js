...
// get 요청 (아무것도 보내지 않으면 get 요청)
// fetch(backendUrl)
// fetch(backendUrl, option)

// 페이지가 아닌 데이터를 요청(DB 접근)했기 때문에 fetch 코드는 "비동기" 코드
...
fetch("https://jsonplaceholder.typicode.com/albums") // return 타입 : Promise (비동기 데이터가 얼마나 걸릴지, 실패 성공 여부 모르기 때문에)
  .then((response) => {
    if(!response.ok){ // 서버 통신 실패했을 때 => 더 이상 서버를 돌릴 필요 없음
  ...
  // .then((result) => {console.log(result)}) // 객체
